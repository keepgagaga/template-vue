<template>
  <div>
      <a :href="res.url">
          <div class="res-item">
              <img class="image" v-bind:src="res.image">
              <div class="des-container">
                  <div class="des-title">{{ res.title }}</div>
                  <div class="des-content">{{ res.description }}</div>
              </div>
              <svg class="share-icon" width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path opacity="0.2" d="M24.5698 1.43021C23.6166 0.476795 22.4682 0 21.1255 0H4.87534C3.53258 0 2.38417 0.476795 1.43057 1.43021C0.477162 2.3838 0.000366211 3.53215 0.000366211 4.87497V21.1249C0.000366211 22.4676 0.477162 23.6161 1.43057 24.5697C2.38417 25.5233 3.53258 26 4.87534 26H21.1253C22.468 26 23.6164 25.5233 24.5696 24.5697C25.5232 23.6161 26 22.4677 26 21.1249V4.87497C26 3.53215 25.5232 2.38362 24.5698 1.43021ZM21.667 13.5415C21.667 14.0155 21.4466 14.3482 21.0068 14.5402C20.8603 14.5966 20.7187 14.6248 20.5835 14.6248C20.2791 14.6248 20.0251 14.5174 19.8218 14.3032L17.3845 11.8658L8.34583 20.9049C8.13115 21.1192 7.87745 21.2266 7.58398 21.2266C7.29038 21.2266 7.03645 21.1192 6.82206 20.9049L5.09553 19.1781C4.88109 18.9641 4.77384 18.7097 4.77384 18.4164C4.77384 18.1233 4.88109 17.8693 5.09553 17.655L14.1347 8.61569L11.6972 6.17824C11.3473 5.85097 11.2684 5.45623 11.4601 4.99337C11.6519 4.55339 11.9849 4.33313 12.4589 4.33313H20.5836C20.8769 4.33313 21.1309 4.44044 21.3456 4.65495C21.5599 4.86922 21.6671 5.12321 21.6671 5.41662V13.5415H21.667Z" fill="#999FAA"/>
              </svg>
          </div>
      </a>
  </div>
</template>

<script>
export default {
    name: 'ResItem',
    props: ['res']
}
</script>

<style>
a {
    text-decoration: none;
    color: #333;
}
.res-item {
    display: flex;
    justify-content: space-between;
    align-items: center;
    width: 690px;
    height: 100px;
    padding-left: 12px;
}
.res-item:hover {
    background-color: #F2F4F8;
    border-radius: 12px;
}
.image {
    width: 58px;
    height: 58px;
    border-radius: 10px;
}
.des-container {
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    justify-content: center;
    margin-left: 20px;
    text-align: left;
}
.des-title {
    font-family: Poppins;
    font-weight: 500;
    font-size: 20px;
    line-height: 26px;
}
.des-content {
    width: 482px;
    height: 20px;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
    color: #999FAA;
    font-family: Poppins;
    font-weight: 400;
    font-size: 16px;
    line-height: 20px;
    letter-spacing: 0.75px;
}
.share-icon {
    margin-right: 20px;
    opacity: 0;
}
.res-item:hover .share-icon {
    opacity: 1;
}
</style>